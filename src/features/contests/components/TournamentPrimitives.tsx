...
export function ContestStatusBadge({ status }: { status: string }) {
  return (
    <Badge
      className={cn(
        "rounded-md border px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wide shadow-none",
...
export function TournamentPanel({
  ...
  return (
    <section
      className={cn(
        "rounded-xl border border-[#e5e2e1] bg-white p-5 shadow-sm",
        className,
      )}
    >
      <div className="mb-5 flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h3 className="text-lg font-extrabold leading-tight text-[#1c1b1b]">
              {title}
            </h3>
            {subtitle ? (
              <p className="mt-1 text-xs font-semibold text-[#5d5f5f]">
                {subtitle}
              </p>
            ) : null}
          </div>
  ...
  return (
    <div className="rounded-lg border border-[#e5e2e1] bg-[#fcf8f8] p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <span className="text-[10px] font-extrabold uppercase tracking-wider text-[#747878]">
          {label}
        </span>
        {icon ? (
          <span className="text-[#747878] [&_svg]:size-4">{icon}</span>
        ) : null}
      </div>
      <div className="text-xl font-black leading-none text-[#1c1b1b]">
        {value}
      </div>
      {helper ? (
        <div className="mt-2 text-xs font-semibold text-[#5d5f5f]">
          {helper}
        </div>
      ) : null}
    </div>
...
export interface ContestFilterOption<T extends string = string> {
  value: T
  label: string
  disabled?: boolean
}

export function ContestToolbar({
  children,
  className,
}: {
  children: ReactNode
  ...
  return (
    <section
      className={cn(
        "rounded-xl border border-[#e5e2e1] bg-white p-4 shadow-sm",
        className,
      )}
    >
...
export function ContestSearchInput({
  value,
  onChange,
  ...
  return (
    <div className={cn("relative w-full", className)}>
      ...
      <Input
        ...
        className={cn(
          "h-11 rounded-lg border-[#e5e2e1] bg-[#fcf8f8] pl-10 text-sm font-semibold text-[#1c1b1b] placeholder:text-[#8b8785] focus-visible:ring-orange-500",
          inputClassName,
        )}
      />
    </div>
...
export function ContestFilterChips<T extends string>({
  options,
  value,
  onChange,
  className,
}: {
  options: readonly ContestFilterOption<T>[]
  value: T
  ...
  return (
    <div
      className={cn("flex w-full gap-2 overflow-x-auto md:w-auto", className)}
    >
      {options.map((option) => (
        <button
          key={option.value}
          ...
          className={cn(
            "shrink-0 rounded-lg px-4 py-1.5 text-xs font-bold transition",
            value === option.value
              ? "bg-orange-600 text-white shadow"
              : "border border-[#e5e2e1] bg-[#fcf8f8] text-[#6f6c6a] hover:text-[#1c1b1b]",
...
export function ContestSelectFilter<T extends string>({
  value,
  onChange,
  options,
  ariaLabel,
  ...
  return (
    <select
      ...
      className={cn(
        "h-11 rounded-lg border border-[#e5e2e1] bg-[#fcf8f8] px-3 text-sm font-bold text-[#444748] outline-none focus:border-orange-500",
        className,
      )}
    >
      {options.map((option) => (
    ...
    <article className="group flex h-full flex-col rounded-xl border border-[#e5e2e1] bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-orange-200 hover:shadow-md">
      ...
      <div className="space-y-2 text-xs font-semibold text-[#444748]">
        ...
        <div className="flex items-center gap-2">
          ...
          <span>
            Đóng đăng ký:{" "}
            {formatContestDateTime(contest.registration_closes_at)}
          </span>
        </div>
        <div className="flex items-start gap-2">
          <MapPin className="mt-0.5 size-4 shrink-0 text-orange-600" />
          <span className="line-clamp-1">
            {contest.participating_cafes?.map((cafe) => cafe.name).join(", ") ||
      ...
      <div className="mt-5 grid grid-cols-3 gap-2">
        <ContestMiniStat
          label="Tham dự"
          value={`${registered}/${contest.capacity}`}
        />
      ...
      <div className="mt-5">
        ...
        <div className="h-2 overflow-hidden rounded-full bg-[#e5e2e1]">
          <div
            className="h-full rounded-full bg-[#1c1b1b]"
            style={{ width: `${percent}%` }}
          />
        </div>
      </div>
...
function ContestMiniStat({
  ...
  return (
    <div className="rounded-lg border border-[#e5e2e1] bg-[#fcf8f8] px-3 py-2">
      <div className="text-[9px] font-extrabold uppercase tracking-wider text-[#747878]">
        {label}
      </div>
      <div className="mt-1 truncate text-xs font-black text-[#1c1b1b]">
        {value}
      </div>
    </div>
  )
...
export function ContestSchedulePanel({
  ...
  if (groups.length === 0) {
    return (
      <TournamentPanel
        title="Trận đấu"
        subtitle="BTC chưa công bố lịch thi đấu"
        icon={<Flag className="size-5" />}
      >
        <EmptyTournamentState
          title="Chưa có trận đấu"
          description="Lịch thi đấu sẽ xuất hiện khi ban tổ chức dựng bracket hoặc tạo heat."
        />
      </TournamentPanel>
    )
  }
  ...
  return (
    <TournamentPanel
      title="Trận đấu"
      subtitle="Lịch thi đấu theo từng vòng"
      icon={<Flag className="size-5" />}
    >
      <div className="grid gap-4 lg:grid-cols-2">
        {groups.map((group) => (
          <div
            ...
            className="rounded-lg border border-[#e5e2e1] bg-[#fcf8f8] p-4"
          >
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                <h4 className="font-extrabold text-[#1c1b1b]">
            ...
            <div className="space-y-2">
              {group.matches.map((match) => (
                <MatchRow
                  key={match.id}
                  match={match}
                  scheduledAt={getRoundScheduledAt(group.round)}
                />
              ))}
            </div>
          </div>
  ...
  return (
    <div className="rounded-lg border border-[#e5e2e1] bg-white p-3">
      <div className="mb-2 flex items-center justify-between text-[10px] font-extrabold uppercase tracking-wider text-[#747878]">
        ...
        <span>
          {match.status === "COMPLETED" ? "KT" : formatContestTime(scheduledAt)}
        </span>
      </div>
      <CompetitorLine
        registration={match.competitorA}
        isWinner={winnerId === match.competitorARegistrationId}
      />
      <CompetitorLine
        registration={match.competitorB}
        isWinner={winnerId === match.competitorBRegistrationId}
      />
    </div>
  )
}
...
function CompetitorLine({
  ...
  return (
    <div
      className={cn(
        "mt-1 flex items-center justify-between rounded-md px-2 py-1.5 text-sm font-bold",
        isWinner ? "bg-orange-50 text-orange-700" : "text-[#1c1b1b]",
      )}
    >
...
export function ContestLeaderboardPanel({
  ...
  standings: ContestLeaderboardStanding[]
}) {
  return (
    <TournamentPanel
      title="Bảng xếp hạng"
      subtitle="Kết quả chỉ tính từ result đã verify"
      icon={<Medal className="size-5" />}
    >
      {standings.length === 0 ? (
        <EmptyTournamentState
          title="Chưa có bảng xếp hạng"
          description="Bảng xếp hạng sẽ hiển thị sau khi ban tổ chức xác nhận kết quả."
        />
      ) : (
          ...
          <table className="w-full min-w-[640px] text-left text-sm">
            ...
            <tbody>
              {standings.map((standing) => (
                <tr
                  key={standing.registration_id}
                  className="border-t border-[#e5e2e1]"
                >
                  <td className="px-4 py-3 font-black text-[#1c1b1b]">
                    #{standing.rank}
                  </td>
                  <td className="px-4 py-3">
                    <div className="font-bold text-[#1c1b1b]">
                      {standing.fullName || "Tay đua RC"}
                    </div>
                  ...
                  <td className="px-4 py-3 font-mono font-bold">
                    {standing.best_lap_ms ?? "--"}
                  </td>
                  <td className="px-4 py-3 font-mono font-bold">
                    {standing.total_time_ms ?? "--"}
                  </td>
...
export function ContestBracketPanel({
  ...
  return (
    <TournamentPanel
      title="Sơ đồ thi đấu"
      subtitle="Winner tự tiến sang vòng tiếp theo"
      icon={<Trophy className="size-5" />}
    >
      {groups.length === 0 ? (
        <EmptyTournamentState
          title="Chưa có bracket"
          description="Bracket sẽ hiển thị sau khi ban tổ chức dựng sơ đồ thi đấu."
        />
      ) : (
        <div className="overflow-x-auto pb-2">
          <div className="flex min-w-[760px] gap-4">
            {groups.map((group) => (
              <div
                key={group.round?.id ?? group.matches[0]?.id}
                className="flex min-w-56 flex-1 flex-col justify-center gap-3"
              >
...
export function ContestRewardPanel({ rewards }: { rewards: ContestReward[] }) {
  return (
    <TournamentPanel
      title="Phần thưởng"
      subtitle="Quà tặng theo thứ hạng chung cuộc"
      icon={<Gift className="size-5" />}
    >
      {rewards.length === 0 ? (
        <EmptyTournamentState
          title="Chưa công bố phần thưởng"
          description="BTC sẽ cập nhật phần thưởng trước ngày thi đấu."
        />
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {rewards.map((reward) => (
            ...
            >
              ...
              <p className="mt-2 text-sm font-semibold leading-relaxed text-[#5d5f5f]">
                {reward.description}
              </p>
            </div>
          ))}
...
export function ContestRegistrationPanel({
  ...
  return (
    <TournamentPanel
      title="Tham gia giải"
      subtitle="Thông tin đăng ký và check-in"
    ...
    >
      <div className="grid grid-cols-2 gap-3">
        <ContestMetric
          label="VĐV"
          value={`${registered}/${contest.capacity}`}
          helper={`${checkedIn} đã check-in`}
          icon={<Users />}
        />
        <ContestMetric
          label="Lệ phí"
          value={formatCurrency(contest.entry_fee)}
          helper="Thanh toán phase sau"
          icon={<Trophy />}
        />
      </div>
      ...
      {myRegistration ? (
        <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-center">
          ...
          <div className="mt-2 text-sm font-extrabold text-emerald-800">
            Bạn đã đăng ký
          </div>
          <div className="mt-3 rounded-lg bg-white p-3">
            <QrCode className="mx-auto size-24 text-[#1c1b1b]" />
            <div className="mt-2 break-all font-mono text-xs font-black text-[#1c1b1b]">
        ...
        <tbody>
          {registrations.map((registration) => (
            <tr key={registration.id} className="border-t border-[#e5e2e1]">
              ...
              <td className="px-4 py-3 font-semibold text-[#5d5f5f]">
                {registrationEmail(registration) || "--"}
              </td>
              <td className="px-4 py-3 font-mono text-xs font-black text-orange-700">
                {registration.check_in_code}
              </td>
              <td className="px-4 py-3 font-bold text-[#444748]">
