...
import { cafeApi, cafeQueryKeys, trackConfigApi, trackTypeApi, trackTypeQueryKeys } from "@/features/cafes/api/cafe.api"
...
export function ProviderContestFormPage() {
  ...
  const [trackConfigsByCafe, setTrackConfigsByCafe] = useState<Record<string, TrackConfig[]>>({})
  ...
  const selectedTemplate = useMemo(
    () => templatesQuery.data?.find((item) => item.id === form.contest_template_id) ?? null,
    [form.contest_template_id, templatesQuery.data],
  )
  const selectedFormat = useMemo(
    () => formatsQuery.data?.find((item) => item.id === form.contest_format_id) ?? null,
  ...
  useEffect(() => {
    ...
    const existingLocks = Array.isArray(contest.config?.resource_locks) ? contest.config.resource_locks : []
    setResourceLocks(
      existingLocks.reduce<ResourceLockState>((map, lock) => {
        const value = lock as { cafe_id?: string; scope?: ResourceLockScope; track_config_ids?: string[] }
        if (!value.cafe_id) return map
        map[value.cafe_id] = {
          scope: value.scope === "SELECTED_TRACKS" ? "SELECTED_TRACKS" : "FULL_BRANCH",
          track_config_ids: Array.isArray(value.track_config_ids) ? value.track_config_ids : [],
        }
        return map
      }, {}),
    )
  ...
  useEffect(() => {
    if (form.participating_cafe_ids.length === 0) {
      setTrackConfigsByCafe({})
      return
    }

    let cancelled = false
    void Promise.all(
      form.participating_cafe_ids.map(async (cafeId) => [cafeId, await trackConfigApi.listTrackConfigs(cafeId)] as const),
    ).then((entries) => {
      if (cancelled) return
  ...
  useEffect(() => {
    ...
    setResourceLocks((current) => {
      ...
      for (const cafeId of form.participating_cafe_ids) {
        const configs = (trackConfigsByCafe[cafeId] ?? []).filter((item) => item.is_active)
        if (configs.length <= 1) {
          next[cafeId] = { scope: "FULL_BRANCH", track_config_ids: configs.map((item) => item.id) }
          continue
        }
        if (!next[cafeId]) {
          ...
          next[cafeId] = {
            ...next[cafeId],
            track_config_ids: next[cafeId].track_config_ids.filter((trackId) => configs.some((item) => item.id === trackId)),
          }
        }
      }
  ...
  const handleSubmit = async () => {
    ...
    const derivedLocks = buildResourceLocks(form.participating_cafe_ids, trackConfigsByCafe, resourceLocks)

    if (derivedLocks.some((item) => item.scope === "SELECTED_TRACKS" && item.track_config_ids.length === 0)) {
      setValidationErrors((current) => ({
        ...current,
        resource_locks: "Vui lòng chọn ít nhất một sân khi khóa theo sân cụ thể.",
      }))
    ...
    const templateDefaults = (selectedTemplate?.defaultConfig ?? {}) as Record<string, unknown>
    const derivedConfig = {
      ...templateDefaults,
      ...extraConfig,
      format: runtimeFormat,
      runtime_format: runtimeFormat,
      leaderboard_mode: runtimeFormat === "KNOCKOUT" ? "KNOCKOUT_WINS" : templateDefaults.leaderboard_mode ?? "BEST_LAP",
      drivers_per_match: runtimeFormat === "KNOCKOUT"
  ...
  return (
    <ProviderShell>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <Panel>
          <PanelTitle title="Thông tin giải đấu" subtitle="Thiết lập loại giải, lịch thi đấu và mốc đăng ký." />
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Tên giải đấu" error={validationErrors["name"]}>
              <Input value={form.name} onChange={(e) => setForm((s) => ({ ...s, name: e.target.value }))} />
            </Field>
            <Field label="Loại đường đua" error={validationErrors["track_type_id"]}>
              <select
            ...
            <Field label="Loại giải" error={validationErrors["contest_type_id"]}>
              <select
                className="h-10 rounded-lg border border-[#c4c7c8] bg-white px-3 text-sm"
            ...
            <Field label="Hình thức thi đấu" error={validationErrors["contest_format_id"]}>
              <select
                className="h-10 rounded-lg border border-[#c4c7c8] bg-white px-3 text-sm"
            ...
            <Field label="Mẫu vận hành" className="md:col-span-2" error={validationErrors["contest_template_id"]}>
              <select
            ...
            <Field label="Bắt đầu thi đấu" error={validationErrors["starts_at"]}>
            ...
            <Field label="Lệ phí tham gia (VND)" error={validationErrors["entry_fee"]}>
              <Input value={form.entry_fee} onChange={(e) => setForm((s) => ({ ...s, entry_fee: e.target.value }))} />
            </Field>
            <Field label="Banner giải đấu" className="md:col-span-2" error={validationErrors["banner_image_url"]}>
        ...
        <div className="space-y-4">
          <Panel>
            <PanelTitle title="Chi nhánh tham gia" subtitle="Chọn chi nhánh và cách khóa tài nguyên cho từng nơi tổ chức." />
            {validationErrors["participating_cafe_ids"] && (
              <p className="mb-2 text-xs font-semibold text-red-500">{validationErrors["participating_cafe_ids"]}</p>
            )}
            {validationErrors["resource_locks"] ? (
              <p className="mb-2 text-xs font-semibold text-red-500">{validationErrors["resource_locks"]}</p>
            ) : null}

            <div className="space-y-3">
              {cafes.map((cafe) => {
                const checked = form.participating_cafe_ids.includes(cafe.id)
                const trackConfigs = (trackConfigsByCafe[cafe.id] ?? []).filter((item) => item.is_active)
                const lockState = resourceLocks[cafe.id] ?? { scope: "FULL_BRANCH" as const, track_config_ids: [] }
                const singleTrack = trackConfigs.length <= 1

                return (
                  <div key={cafe.id} className="rounded-lg border border-[#e5e2e1] p-3">
                      ...
                      <div className="mt-3 rounded-lg bg-[#fcf8f8] p-3">
                        {singleTrack ? (
                          <p className="text-sm font-semibold text-[#5d5f5f]">
                            Chi nhánh này hiện chỉ có một sân hoạt động. Hệ thống sẽ tự khóa toàn bộ chi nhánh trong khung giờ giải đấu.
                          </p>
                        ) : (
                          <div className="space-y-3">
                            <div className="grid gap-3 md:grid-cols-2">
                              <label className="rounded-lg border border-[#e5e2e1] bg-white px-3 py-2 text-sm font-semibold text-[#1c1b1b]">
                                <input
                                  ...
                                  onChange={() =>
                                    setResourceLocks((current) => ({
                                      ...current,
                                      [cafe.id]: { scope: "FULL_BRANCH", track_config_ids: [] },
                                    }))
                                  }
                                />
                              ...
                              <label className="rounded-lg border border-[#e5e2e1] bg-white px-3 py-2 text-sm font-semibold text-[#1c1b1b]">
                                <input
                                  type="radio"
                                  className="mr-2"
                                  checked={lockState.scope === "SELECTED_TRACKS"}
                                  onChange={() =>
                                    setResourceLocks((current) => ({
                                      ...current,
                                      [cafe.id]: {
                                        scope: "SELECTED_TRACKS",
                                        track_config_ids: current[cafe.id]?.track_config_ids ?? [],
                                      },
                                    }))
                                  }
                                />
                            ...
                            {lockState.scope === "SELECTED_TRACKS" ? (
                              <div className="space-y-2">
                                {trackConfigs.map((trackConfig) => {
                                  const trackChecked = lockState.track_config_ids.includes(trackConfig.id)
                                  return (
                                    <label key={trackConfig.id} className="flex items-start gap-3 rounded-lg border border-[#e5e2e1] bg-white px-3 py-2">
                                      <input
                                        type="checkbox"
                                          ...
                                          setResourceLocks((current) => {
                                            ...
                                            return {
                                              ...
                                              [cafe.id]: {
                                                ...existing,
                                                scope: "SELECTED_TRACKS",
                                                track_config_ids: e.target.checked
                                                  ? [...existing.track_config_ids, trackConfig.id]
                                                  : existing.track_config_ids.filter((id) => id !== trackConfig.id),
                                              },
                                            }
                                          })
                                        }
                                      ...
                                      <div>
                                        <p className="text-sm font-bold text-[#1c1b1b]">
                                          {trackConfig.track_type?.name ?? "Sân thi đấu"}
                                        </p>
                                        <p className="text-xs font-medium text-[#747878]">
                                          Tối đa {trackConfig.max_concurrent} lượt thuê cùng lúc
                                        </p>
                                      </div>
                                    </label>
                                  )
                              ...
                              <p className="text-sm font-semibold text-[#5d5f5f]">
                                Mọi booking mới trong khung giờ giải đấu tại chi nhánh này sẽ bị chặn để dành tài nguyên cho giải.
                              </p>
                            )}
                          </div>
                        )}
          ...
          <Panel>
            <PanelTitle title="Quy tắc vận hành" subtitle="Thiết lập xe thi đấu và tóm tắt cách hệ thống sẽ vận hành contest." />
            <div className="space-y-4">
              <Field label="Nguồn xe thi đấu" error={validationErrors["vehicle_rule.vehicle_policy"]}>
                <select
                  className="h-10 w-full rounded-lg border border-[#c4c7c8] bg-white px-3 text-sm"
              ...
              <Field label="Thời điểm gán xe" error={validationErrors["vehicle_rule.assignment_policy"]}>
                <select
                  className="h-10 w-full rounded-lg border border-[#c4c7c8] bg-white px-3 text-sm"
                ...
                >
                  ...
                  <option value="PRE_ASSIGNED">Gán xe trước khi check-in</option>
                </select>
              </Field>

              <div className="rounded-lg border border-[#e5e2e1] bg-[#fcf8f8] p-4">
                <p className="text-sm font-bold text-[#1c1b1b]">Tóm tắt vận hành</p>
                <div className="mt-3 space-y-2 text-sm font-semibold text-[#5d5f5f]">
                  ...
                  <p>Luồng bắt buộc: Đăng ký → Duyệt → Check-in → Xếp nhánh → Thi đấu</p>
          ...
          {isEdit ? (
            <Panel>
              <PanelTitle title="Vận hành giải đấu" subtitle="Màn này chỉ giữ phần cấu hình. Mọi thao tác tiếp nhận, check-in, xếp nhánh và nhập kết quả nằm ở khu vận hành riêng." />
              <div className="rounded-lg border border-dashed border-[#c4c7c8] bg-[#fcf8f8] p-6">
                <p className="text-sm font-semibold text-[#5d5f5f]">
                  Sau khi lưu cấu hình, hãy chuyển sang màn vận hành để tiếp nhận người chơi, check-in, tạo nhánh đấu và nhập kết quả.
                </p>
                <Button
                  type="button"
...
function getRuntimeFormatFromCode(code?: string | null): "TIME_TRIAL" | "KNOCKOUT" {
  return code === "TIME_TRIAL" ? "TIME_TRIAL" : "KNOCKOUT"
}

function stripManagedContestConfig(config: Record<string, unknown> | null | undefined) {
...
function buildResourceLocks(
  ...
  return cafeIds.map((cafeId) => {
    const trackConfigs = (trackConfigsByCafe[cafeId] ?? []).filter((item) => item.is_active)
    ...
    if (lockState?.scope === "SELECTED_TRACKS") {
      return {
        cafe_id: cafeId,
        scope: "SELECTED_TRACKS" as const,
        track_config_ids: lockState.track_config_ids.filter((trackId) => trackConfigs.some((item) => item.id === trackId)),
      }
    }
