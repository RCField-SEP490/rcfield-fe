...
export function PaymentStep({ }: PaymentStepProps) {
  return (
    <Card className="rounded-xl shadow-sm">
      <CardHeader>
        <CardTitle>Phương thức thanh toán</CardTitle>
        <p className="text-sm text-muted-foreground">Xác nhận đơn đặt lịch và thanh toán qua cổng VNPay.</p>
      </CardHeader>
      <CardContent>
        <div className="flex items-center gap-4 rounded-xl border-2 border-primary bg-primary/5 p-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">Quét QR hoặc thanh toán qua ví điện tử VNPay</p>
          </div>
        </div>
        <p className="mt-3 text-xs text-muted-foreground">
          Bấm "Xác nhận thanh toán" để chuyển đến trang thanh toán VNPay{isSandbox ? " (môi trường thử nghiệm)" : ""}.
        </p>
      </CardContent>
    </Card>
  )
