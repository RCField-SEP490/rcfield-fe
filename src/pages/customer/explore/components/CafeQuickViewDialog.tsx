        ...
        {displayCafe && (
          <>
            <DialogHeader className="sr-only">
              ...
              <DialogDescription>Xem nhanh thông tin cơ sở RCField.</DialogDescription>
            </DialogHeader>
            <div className="relative h-64 overflow-hidden bg-slate-100">
              ...
              <div className="absolute bottom-5 left-5 right-5 text-white">
                <div className="mb-2 flex flex-wrap gap-2">
                  {displayCafe.status && <Badge className="bg-white/90 font-black text-slate-950">{formatStatus(displayCafe.status)}</Badge>}
                  {loadingDetail && <Badge className="bg-white/80 font-black text-slate-700">Đang cập nhật...</Badge>}
                  {displayCafe.features.includes("Serious Inspection") && <Badge className="bg-emerald-600 font-black text-white"><ShieldCheck className="h-3 w-3" /> Serious Inspection</Badge>}
                </div>
                <h2 className="text-2xl font-black tracking-tight">{displayCafe.name}</h2>
                <p className="mt-1 flex items-center gap-1 text-sm font-bold text-slate-200"><MapPin className="h-4 w-4 text-orange-400" /> {displayCafe.address}</p>
              </div>
            </div>
            <div className="max-h-[calc(90svh-16rem)] overflow-y-auto p-5">
              <div className="grid gap-5 md:grid-cols-[1fr_260px]">
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="mt-2 text-sm font-medium leading-6 text-slate-600">{displayCafe.description}</p>
                  </div>
                  <div>
                    <p className="text-xs font-black uppercase tracking-wide text-slate-400">Track</p>
                    <div className="mt-2 flex flex-wrap gap-2">
                      {displayCafe.trackTypes.length > 0 ? (
                        displayCafe.trackTypes.map((item) => <Badge key={item} variant="secondary" className="bg-slate-100 font-bold text-slate-700">{item}</Badge>)
                      ) : (
                        <span className="text-sm font-medium text-slate-500">Chưa cập nhật loại track.</span>
                      )}
                    </div>
                  </div>
                  <div className="grid gap-2 sm:grid-cols-2">
                    <QuickInfo icon={MapPin} label="Khu vực" value={`${displayCafe.district}, ${displayCafe.city}`} />
                    <QuickInfo icon={Phone} label="Liên hệ" value={displayCafe.phone || "Chưa cập nhật"} />
                    <QuickInfo icon={Clock3} label="Thời lượng slot" value={`${displayCafe.slotDurationMinutes ?? 60} phút`} />
                    <QuickInfo icon={Users} label="Sức chứa BYOC" value={displayCafe.byocCapacity !== undefined ? `${displayCafe.byocCapacity}` : "Chưa cập nhật"} />
                  </div>
                </div>
              ...
              {displayCafe.availableVehicles.length > 0 ? (
                <div className="mt-6">
                  ...
                  <VehicleMiniList cafeId={displayCafe.id} vehicles={displayCafe.availableVehicles} onBookNow={onBookNow} />
                </div>
              ) : (
...
function QuickInfo({ icon: Icon, label, value }: { icon: typeof MapPin; label: string; value: string }) {
  return (
